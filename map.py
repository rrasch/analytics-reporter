...
#shapefile = 'merge.shp'
shapefile = os.path.expanduser('~/Downloads/ne_10m_admin_0_map_units/ne_10m_admin_0_map_units.shp')
...
#gdf = gdf[['ADM0_A3', 'geometry']].to_crs('+proj=robin')
gdf = gdf[['ISO_A3', 'geometry']].to_crs('+proj=robin')
...
merged = gdf.merge(df, left_on='ISO_A3', right_on='iso3')
...
"""
#merged.dropna().plot(ax=ax, column='sessions', cmap=cmap, scheme='equal_interval', k=colors, legend=True)
#merged.dropna().plot(ax=ax, column='sessions', cmap=cmap, scheme='percentiles', k=colors, legend=True, legend_kwds={'loc': 'lower left'})
#merged.dropna().plot(ax=ax, column='sessions', cmap=cmap, edgecolor='white', linewidth=0,  scheme='quantiles', k=colors, legend=True, legend_kwds={'loc': 'lower left'})
...
#merged.dropna().plot(ax=ax, column='sessions', cmap=cmap)
